...
def write_resident(root: Path, *, affine_shared: bool = False) -> None:
    ...
    for component in ("gate_proj", "up_proj", "down_proj"):
        ...
        if affine_shared:
            weight = struct.pack("<8I", *([pack8(1)] * 8))
            scales = bf16(1.0) * 8
            biases = bf16(0.0) * 8
            for suffix, data, dtype, shape in (
                ("weight", weight, "U32", [8, 1]),
                ("scales", scales, "BF16", [8, 1]),
                ("biases", biases, "BF16", [8, 1]),
            ):
                tensors.append(
                    {
                        "name": f"{stem}.{suffix}",
...
def expected_routed() -> float:
    expert0 = 512.0 / (1.0 + math.exp(-8.0))
    expert1 = 4096.0 / (1.0 + math.exp(-16.0))
    return (2.0 / 3.0) * expert1 + (1.0 / 3.0) * expert0


def expected_shared() -> float:
    return 512.0 / (1.0 + math.exp(-8.0))


def run_case(root: Path, *, affine_shared: bool) -> None:
    write_experts(root)
    write_resident(root, affine_shared=affine_shared)
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="largerlm-shared-layer-moe-", dir="/private/tmp"))
    run_case(root / "f32", affine_shared=False)
    run_case(root / "affine", affine_shared=True)
    expected = expected_routed() + expected_shared()
    print(f"fixture: {root}")
    print(f"  expected output[0]: {expected:.6f}")
    print("  affine shared:      ok")
    print("  shared smoke:       ok")
    return 0

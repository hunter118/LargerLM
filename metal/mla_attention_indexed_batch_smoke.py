...
def reference(
    ...
    *,
    ...
    output: list[float] = []
    for token, row_indices in enumerate(indices):
...
def write_fixture(root: Path) -> tuple[list[float], list[float], list[list[float]], list[list[float]], list[list[int]]]:
    ...
    (root / "indices.u32").write_bytes(struct.pack("<6I", 2, 1, 0, 2, 2, 0))
    return q_nope, q_rope, cache, kv_b, indices


def run_case(root: Path, expected: list[float], interleave: bool) -> None:
    output = root / ("indexed_attn_interleave.f32" if interleave else "indexed_attn_default.f32")
    cmd = [
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="largerlm-mla-attn-indexed-", dir="/private/tmp"))
    q_nope, q_rope, cache, kv_b, indices = write_fixture(root)
    common = {
        "num_heads": 2,
        "kv_lora_dim": 2,
        "qk_nope_dim": 1,
    ...
    run_case(
        root,
        reference(q_nope, q_rope, cache, kv_b, indices, interleave=False, **common),
        interleave=False,
    )
    run_case(
        root,
        reference(q_nope, q_rope, cache, kv_b, indices, interleave=True, **common),
        interleave=True,
    )
    print(f"fixture: {root}")
    print("  mla attention indexed batch: ok")
    return 0

...
def run_case(root: Path, expected: list[float], interleave: bool) -> None:
    output = root / ("attn_interleave.f32" if interleave else "attn_default.f32")
    cmd = [
...
def main() -> int:
    root = Path(tempfile.mkdtemp(prefix="largerlm-mla-attn-", dir="/private/tmp"))
    ...
    expected_default = reference(
        q_nope, q_rope, cache, kv_b, interleave=False, **common
    )
    expected_interleave = reference(
        q_nope, q_rope, cache, kv_b, interleave=True, **common
    )
    run_case(root, expected_default, interleave=False)
    run_case(root, expected_interleave, interleave=True)
    print(f"fixture: {root}")
    print("  mla attention:      ok")
    return 0
